    ...
    def __init__(self, prompt_path, concept):
        ...
        with open(prompt_path, "r", encoding='utf-8') as f:
            for anchor in f.readlines():
                ...
                if anchor_concept not in concept:
...
def train_esd(prompts,
    ...
    ----------
    prompts : List[str]
        The list of prompts corresponding to the concept to erase from diffusion model.
    concept_name : str
        Name of the concept (used for naming the saved model / loss curve).
    train_method : str
    ...
    # PROMPT CLEANING
    word_print = concept_name
    surrogate_concept_print = surrogate_concept

    words = [word.strip() for word in prompts]
    # map each diverse prompt p (= c_a + c) to its concept-free version c_a
    # (the `target_prompt` column). ESD erases p toward c_a instead of toward "".
    if target_prompts is not None:
        prompt_to_target = {p.strip(): t for p, t in zip(prompts, target_prompts)}
    else:
        prompt_to_target = None
    print(words)
    ...
    for i, data in zip(pbar, anchor_dataloader):
        ...
        # print(emb_n.shape)
        # concept-free target c_a (target_prompt) used as the ESD anchor for this word.
        if prompt_to_target is not None:
            emb_ca = embeds_prompt(prompt_to_target[word],
                                   text_encoder=model.text_encoder,
                                   tokenizer=model.tokenizer,
                                   attention_mask=None,
                                   text_encoder_use_attention_mask=False
            ...
            emb_ca = emb_0
        # fixed original concept c for the unconditional (UEG) branch.
        # ESD erases the diverse prompt `word`, but the unconditional loss is kept
        # unchanged and always targets the fixed concept c (passed via --concept).
        if concept is not None:
            emb_c = embeds_prompt([concept],
                                  text_encoder=model.text_encoder,
                                  tokenizer=model.tokenizer,
                                  attention_mask=None,
                                  text_encoder_use_attention_mask=False
        ...
        with torch.no_grad():
            ...
            # frozen conditional prediction for the FIXED concept c.
            # Used only as the target-pull direction of the unconditional loss.
            if is_train_null and concept is not None:
                e_c = train_util.predict_noise(
                    model.unet,
                    scheduler_ori,
                    current_timestep,
                    latent_t,
                ...
                e_c = e_p
            # frozen conditional prediction for the concept-free target c_a.
            # Used as the anchor of the ESD loss (erase p toward c_a instead of "").
            if prompt_to_target is not None:
                e_ca = train_util.predict_noise(
                    model.unet,
                    scheduler_ori,
                    current_timestep,
                    latent_t,
        ...
        e_ca.requires_grad = False

        if is_train_null:
            if no_certain_sur:
                surrogate_guidance = torch.zeros_like(latent_t)
                if is_sc_clip:
                    # gamma_p normalized w.r.t. the FIXED concept c (unchanged UEG),
                    # not the sampled diverse prompt.
                    own_clip = sc_clip[0][concept if concept is not None else word]
            ...
            # unconditional loss keeps targeting the fixed concept c via e_c
            loss_erase_null = criteria(e_n_0,
                                       e_0 + negative_guidance_scale * (
                                                   e_c - e_0) - surrogate_guidance_scale * surrogate_guidance)

        if not no_cond:
            # ESD: erase p toward c_a (e_ca) instead of toward unconditional "".
            loss_erase_cond = criteria(e_n,
                                       e_ca - (negative_guidance_scale * (e_p - e_ca)))
        if is_train_null:
            if no_cond:
...
def main(args):
    ...
    target_prompts = prompt_df["target_prompt"].tolist() if "target_prompt" in prompt_df.columns else None
    concept_name = os.path.splitext(os.path.basename(args.prompt_csv))[0]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--concept', help='original concept c targeted by the unconditional (UEG) loss; '
                                           'the diverse prompts in the csv are c_a + c. If omitted, falls back to '
                                           'the sampled diverse prompt (original diverse behaviour).',
