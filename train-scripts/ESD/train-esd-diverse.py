...
import pandas as pd
def get_to_be_erased_prompt(word, prompt_csv):

    df = pd.read_csv(prompt_csv)
    df = df[df['keyword'] == word] # Filter for each keyword
    # random sample a row from the dataframe
    row = df.sample(1)
    assert word in row['keyword'].values[0], f'{word} not in {row["keyword"].values[0]}'
    prompt = row['prompt'].values[0]
    target_prompt = row['target_prompt'].values[0]
    
    return prompt, target_prompt

def train_esd(prompt, train_method, start_guidance, negative_guidance, iterations, lr, config_path, ckpt_path, diffusers_config_path, devices, prompt_csv, level, seperator=None, image_size=512, ddim_steps=50, info=''):
    '''
    ...
    for i in pbar:
        ...
        # CHANGE HERE
        # get to-be-erased prompt from a csv file 
        to_be_erased_prompt, target_prompt = get_to_be_erased_prompt(word, prompt_csv)

        # get text embeddings for unconditional and conditional prompts
        emb_target = model.get_learned_conditioning([target_prompt])
        emb_erased = model.get_learned_conditioning([to_be_erased_prompt])
        emb_preserved = model.get_learned_conditioning([target_prompt])
        ...
        with torch.no_grad():
            ...
            z_erased = quick_sample_till_t(emb_erased.to(devices[0]), start_guidance, start_code, int(t_enc)) # emb_erased seems to work better instead of emb_target
            # get conditional and unconditional scores from frozen model at time step t and image z
            e_target_org = model_orig.apply_model(z_erased.to(devices[1]), t_enc_ddpm.to(devices[1]), emb_target.to(devices[1]))
            e_erased_org = model_orig.apply_model(z_erased.to(devices[1]), t_enc_ddpm.to(devices[1]), emb_erased.to(devices[1]))
            e_preserved_org = model_orig.apply_model(z_erased.to(devices[1]), t_enc_ddpm.to(devices[1]), emb_preserved.to(devices[1]))

        # breakpoint()
        # get conditional score from ESD model
        e_erased = model.apply_model(z_erased.to(devices[0]), t_enc_ddpm.to(devices[0]), emb_erased.to(devices[0]))
        e_preserved = model.apply_model(z_erased.to(devices[0]), t_enc_ddpm.to(devices[0]), emb_preserved.to(devices[0]))
        e_target_org.requires_grad = False
        e_erased_org.requires_grad = False
        ...
        loss = criteria(e_erased.to(devices[0]), e_target_org.to(devices[0]) - (negative_guidance*(e_erased_org.to(devices[0]) - e_target_org.to(devices[0])))) #loss = criteria(e_n, e_0) works the best try 5000 epochs
